...
// FLS - Make gradient convolution mask
...
// 	}
	...
	// FLS Initialize Input
	srand(time(0));
	for (int i=0; i<inputSignalWidth; i++) {
		for (int x=0; x<inputSignalHeight; x++) {
			inputSignal[i][x] = (float)(rand()%10);
		}
	}
    ...
    for (int y = 0; y < outputSignalHeight; y++)
	{
		...
		{
			std::cout << std::setw(6) << outputSignal[y][x] << " ";
		}
		std::cout << std::endl;
	}
	...
	// FLS
	// Extract profiling event timing information
	// Reference: https://stackoverflow.com/questions/23550912/
