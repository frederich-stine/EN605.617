...
//============================================================================
__device__ void copyToShared(const int *const global_a,
                             const int *const global_b, int *shared_a,
                             int *shared_b) {
...
__global__ void run_gpu(int *a, int *b, int *c, int Module) {
    switch (Module) {
    case 5:
        // Not giving anything away here but presume that shared memory is
        // allocated correctly
...
int main(int argc, char *argv[]) {
	...
	...
	// Populate our arrays with numbers.  - Presumed that below is done
	// correctly
	for (int i = 0; i < N; i++){...}
...
//============================================================================
void speedTest(int *a, int *b, int *c, int *deva, int *devb, int *devc,
               int bytes, int Module) {
