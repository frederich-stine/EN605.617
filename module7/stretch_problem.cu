...
void print_all_device_properties(int device_id){
	cudaDeviceProp prop;
	cudaError_t	ret;
	// FLS Fix some error checking here that was causing failures
	ret = cudaGetDeviceProperties( &prop, device_id);
	if (ret != cudaSuccess) {
		printf("Error: cudaGetDeviceProperties failed with %s\r\n", cudaGetErrorString(ret));
		printf("======================================\r\n");
		return;
	}
...
int main(void) {
	...
	//print out the results of tme time executions returned by the prev methods
	printf("========================\n");
	printf("Summary\n");
